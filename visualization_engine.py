...
for current_argument, current_value in arguments:
    ...
    if current_argument in ('-s', '--datasource'):
        ...
        print("Use -d or --database to set the InfluxDB database name, \
            if not set the default name will be used (homebrew). \
            Example: -d BakerStreet")
        print("Use -s or --datasource to set the Grafana datasource name, \
            if not set the default name will be used (homebrew). \
            Example: -d BakerStreet")
        exit(2)

try:
    print('Connecting to InfluxDB on host: ',
          influxdb_host, ' port: ', influxdb_port)
    client = InfluxDBClient(influxdb_host, influxdb_port)
    replaceInFile("dashboard.json",
    ...
    if (f"'name': '{datasource_name}'" in str(r.json())):
        ...
        r = requests.post(url, auth=HTTPBasicAuth('admin', 'admin'),
                          data=open('datasource.json', 'rb'), headers=headers)
    ...
    headers = {'Accept': 'application/json', 'Content-Type':
               'application/json'}
    r = requests.post(url, auth=HTTPBasicAuth('admin', 'admin'),
                      data=open('dashboard.json', 'rb'), headers=headers)
except getopt.error as err:
    error(str(err))
