...
usage() {
  ...
  echo "Use -s or --datasource to set the Grafana datasource name, if not set the default name will be used (InfluxDB). Example: -s Gerry"
  exit 2
}
...
if [[ output=$(echo "deb https://packages.grafana.com/oss/deb stable main" | sudo tee -a /etc/apt/sources.list.d/grafana.list) -ne 0 ]]; then
  error "Unable to add Grafana Repository to the source list.\n${output}"
fi

echo "Installing InfluxDB & Grafana"
apt update
if [[ output=$(apt -y install influxdb grafana) -ne 0 ]]; then
...
if [[ output=$(pip3 install influxdb) -ne 0 ]]; then
